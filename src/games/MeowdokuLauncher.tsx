// molar-experience 0.9.5 integration: host-local Meowdoku launcher.
//
// Appointment production already had Meowdoku before this release (see
// `src/VirtualPet/components/GamePage.tsx`, the legacy pre-migration
// multi-game shell). This file extracts ONLY that game's iframe/
// postMessage behavior — not the old GamePage's generic multi-game
// chrome/Flappy/Pac-Cat/Tetris handling, which now lives entirely inside
// `@mrburdeveloperteam/molar-experience/pet`'s `SharedVirtualPet`.
//
// Coin persistence is routed exclusively through `appointmentsPetRepository`
...
// (VirtualPet/components/GamePage.tsx's own GAME_CONFIG.meowdoku.url).
...
interface MeowdokuLauncherProps {
   ...
   *  identity boundary (same value passed to AppointmentsVirtualPet).
   *  `null` means signed-out; Meowdoku then runs read-only/local-
   *  progress-only, matching the pre-migration source's own guest
   *  handling. */
  userId: string | null;
}
...
export default function MeowdokuLauncher({ isOpen, onClose, userId }: MeowdokuLauncherProps) {
  ...
  useEffect(() => {
    ...
    const sendWallet = async () => {
      ...
      try {
        const snapshot = await appointmentsPetRepository.loadSnapshot(userId);
    ...
    const spendMeowdokuCoins = async (data: { amount?: unknown; requestId?: unknown }) => {
      ...
      try {
        ...
        await appointmentsPetRepository.mutateCoins(userId, -amount);
    ...
    const applyMeowdokuReward = async (data: { coins?: unknown }) => {
      ...
      try {
        await appointmentsPetRepository.mutateCoins(userId, reward);
      } catch (err) {
      ...
      try {
        const snapshot = await appointmentsPetRepository.loadSnapshot(userId);
        if (snapshot) {
          await appointmentsPetRepository.saveSnapshot({
            ...snapshot,
            stats: {
              ...snapshot.stats,
