...
// in the shared package — confirmed byte-identical to Appointments' own
...
// XP_TO_LEVEL_UP, ROOM_THEMES, LANDSCAPE_GAME_IDS, soap/soap2 exclusion,
// toy-quantity clamp, and game asset paths — see also the fresh Phase 8E
// audit note on the two Appointments-only cache-busting query strings on
// the flappy-cat/tetris game URLs, which the shared package's bundled
// GAME_CONFIG does not carry; flagged for manual acceptance, not fixable
// from the host since SharedVirtualPet owns the game shell entirely).
//
// What stays here, unchanged from the old `VirtualPet/VirtualPetContainer.tsx`,
// is exactly what's genuinely Appointments-specific and Supabase-coupled:
...
//     `VirtualPetContainer` already did — the old `App.jsx` never threaded
...
//
// GLOBAL PET BACKEND FREEZE: `userId` below is Appointments' own
...
export default function AppointmentsVirtualPet({ isOpen, onClose, userId, extraGames }: AppointmentsVirtualPetProps) {
