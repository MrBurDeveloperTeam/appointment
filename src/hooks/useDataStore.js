...
export default function useDataStore(activeClinicId, enabled = true) {
  ...
  // ACTUALLY fetched successfully, set together with a successful result
  // — distinct from `dateRange` (the currently REQUESTED range, which may
  // already have moved on to a new value while a fetch for an older range
  // is still in flight or just failed).
  ...
  // write to `appointments`/`appointmentDataStatus`/`loadedAppointmentRange`
  // — without one shared counter, an older in-flight request from either
  // flow could resolve after a newer one and overwrite current state with
  // stale data. Every fetch increments this ref when it STARTS; only the
  // fetch whose captured id still matches `appointmentRequestIdRef.current`
  // when it resolves is allowed to write state.
