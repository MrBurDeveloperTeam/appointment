 ...
 * App Gallery, Inventory, and To-Do pattern. Given a family's ORDERED
 * candidate pool (business ordering already applied by the provider — see
 * e.g. appointmentSoonProvider.ts's evaluateAppointmentSoonCandidates),
 * returns the first candidate that is neither already shown THIS MOUNT
 * nor explicitly dismissed for `userId`, or `null` if every candidate in
 * the pool is currently ineligible.
 *
